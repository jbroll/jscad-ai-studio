...
export const agentCommand = (agent, model) => {
  const prompt = `Read AGENTS.md and start on ${model}`;
  return agent === "opencode"
    ? { command: "opencode", args: ["--prompt", prompt] }
    : { command: "claude", args: [prompt] };
};

export const runAgentSession = (workspace, model, agent) => {
  const { command, args } = agentCommand(agent, model);
  return spawnSync(command, args, { cwd: workspace, stdio: "inherit" });
};
...
export const runInit = async (
  ...
  const agent = args.includes("--opencode") ? "opencode" : "claude";
  const modelArg = args.find((a) => a !== "--force" && a !== "--opencode");
  ...
  const result = runIgnoringSignals(() => runAgent(workspace, scaffold.model, agent));
  if (result.error?.code === "ENOENT") {
    const { command } = agentCommand(agent, scaffold.model);
    log(cfg.viewerUrl);
    log(`${command} not found; the server keeps running; stop it with jscad-work stop`);
    return 0;
  }
