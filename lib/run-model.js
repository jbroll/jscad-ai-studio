...
import { measureAnchors, measureBetween, measureGeom, measureParts } from "./measure.js";
...
export const runModelSync = (modelPath, opts = {}) => {
  ...
  const { interference, anchors } = opts;
  ...
  if (outputs.includes("measure")) {
    ...
    if (anchors) result.measure.anchors = measureAnchors(run.geom, run.geomType);
