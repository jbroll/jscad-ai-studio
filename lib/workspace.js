...
export const agentsMd = () => `# JSCAD AI Studio workspace

On startup, before anything else:

1. **The work server is running.** jscad-work init started the work server and opened the browser before this session started. The viewer URL is in \`JSCAD.md\`. If \`.jscad-studio\` is missing or its pid is dead, ask the user to rerun \`jscad-work init\`.
...
export const scaffoldWorkspace = (cwd, model, { force = false } = {}) => {
  ...
  if (!extname(modelName)) modelName = `${modelName}.js`;
