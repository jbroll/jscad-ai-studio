...
1. After every edit: \`jscad-work eval <model>\` exits 0, then \`jscad-work measure <model>\`; compare \`dimensions\` with the target from the user or \`NOTES.md\`. Report target and measured values. When \`<model name>.spec.json\` exists, \`jscad-work verify-spec <model>\` must exit 0; once targets are met, \`verify-spec <model> --write\` records them.
2. Before saying a change is done: \`jscad-work check <model> --bed X,Y,Z\` (\`empty: false\`, \`watertight: true\`, \`manifold: true\`, \`fitsBed: true\`, \`selfIntersecting: false\`; for an array, every item in \`items\`, and \`jscad-work interference <model>\` lists no overlap beyond those passed with \`--allow\`). For a printed part, \`jscad-work dfm <model> --wall 1.2\` in its print orientation (\`--up\`) has no \`thinRegions\` and no \`overhangRegions\` except ones you plan to support or bridge; note those in \`NOTES.md\`. Then \`jscad-work render <model> --view all\` and Read every PNG (\`front\`, \`back\`, \`left\`, \`right\`, \`top\`, \`bottom\`, \`iso\`). For internal features (holes, bores, pockets, hollows), also \`render <model> --section z,OFFSET --view top\` (or \`y\` with \`front\`) through them and Read it.
...
// Unpublished packages need these two variables until jscad-anchors/jscad-fluent
// are published and the jscadui viewer is deployed; init writes them when the
// sibling builds exist so Claude Code sessions pick them up automatically.
export const ensureLocalPackageEnv = (cwd, { siblingRoot = resolve(STUDIO_ROOT, "..") } = {}) => {
  const path = resolve(cwd, ".claude/settings.json");
  const existed = existsSync(path);
  let settings = {};
  if (existed) {
    try {
      ...
      throw new Error(
        "settings.json is not valid JSON; add JSCAD_VIEWER_ROOT/JSCAD_LOCAL_PACKAGES to env by hand",
      );
    }
  }
  ...
  if (
    ...
    throw new Error(
      `${path} has an unexpected shape; add JSCAD_VIEWER_ROOT/JSCAD_LOCAL_PACKAGES to env by hand`,
    );
  }

  const viewerIndex = resolve(siblingRoot, "jscadui/apps/jscad-web/build/index.html");
  const anchorsDist = resolve(siblingRoot, "jscad-anchors/dist/jscad-anchors.cjs");
  const fluentDist = resolve(siblingRoot, "jscad-fluent/dist/jscad-fluent.umd.cjs");

  const results = [];
  let changed = false;

  if ("JSCAD_VIEWER_ROOT" in env) {
    results.push({ name: "JSCAD_VIEWER_ROOT", status: "kept" });
  } else if (existsSync(viewerIndex)) {
    env.JSCAD_VIEWER_ROOT = dirname(viewerIndex);
    results.push({ name: "JSCAD_VIEWER_ROOT", status: "set", value: env.JSCAD_VIEWER_ROOT });
    changed = true;
  } else {
    results.push({ name: "JSCAD_VIEWER_ROOT", status: "missing", files: [viewerIndex] });
  }

  if ("JSCAD_LOCAL_PACKAGES" in env) {
    results.push({ name: "JSCAD_LOCAL_PACKAGES", status: "kept" });
  } else if (existsSync(anchorsDist) && existsSync(fluentDist)) {
    env.JSCAD_LOCAL_PACKAGES = [dirname(dirname(anchorsDist)), dirname(dirname(fluentDist))].join(
      ",",
    );
    results.push({
      name: "JSCAD_LOCAL_PACKAGES",
      status: "set",
      value: env.JSCAD_LOCAL_PACKAGES,
    });
    changed = true;
  } else {
    results.push({
      name: "JSCAD_LOCAL_PACKAGES",
      status: "missing",
      files: [anchorsDist, fluentDist].filter((f) => !existsSync(f)),
    });
  }
...
export const scaffoldWorkspace = (cwd, model, { force = false } = {}) => {
  ...
  return {
    ...
    created,
    kept,
    allowRule: ensureAllowRule(cwd).status,
    localPackageEnv: ensureLocalPackageEnv(cwd).results,
  };
};
