...
test("renders a PNG of the model at the requested size", async () => {
...
test("renders several views from one page load", async () => {
...
test("decodes real viewer PNGs and pixel-diffs two views", async () => {
...
test("renders a section that differs from the whole model and removes its wrapper", async () => {
...
test("reports a section offset outside the model as a viewer error", async () => {
  await expect(
...
test("reports a model that throws in the viewer instead of capturing an empty scene", async () => {
  await expect(renderModel(fx("broken.js"), { size: [400, 300] })).rejects.toThrow(
    /model error in viewer: .*nonExistentMethod/,
  );
}, 60000);

test("names the timeout when the viewer does not finish", async () => {
