...
`jscad-work init [model.js|directory] [--opencode]` is the one command that gets a workspace running: `claude` starts inside it with the browser already open on the model. `--opencode` starts `opencode` instead.

**Argument.** A directory argument (existing) becomes the workspace; its model is the first `.js` file there, or a new `model.js` from the template. Any other argument is a model path: its dirname becomes the workspace (which must already exist) and its basename the model, `.js` added if missing and the file created from the template if it does not exist. No argument uses the current directory. A relative path resolves against the current directory. A directory argument that does not exist is an error (`no such directory`), including one written with a trailing slash (`init sub/` when `sub` does not exist).

A model name ending in `.scad` is used as is: an existing `part.scad` works unchanged, but a missing one is an error (`no such file`), since init has no OpenSCAD starter template. A dotted name with any other extension is also an error (`unsupported model extension`); name it with `.js` or `.scad`, for example `bracket.v2.js` instead of `bracket.v2`.
...
2. Starts the work server in the background if `.jscad-studio` does not already name a live one; a live one is reused and left running. The server's stdout and stderr go to `.jscad-work.log` in the workspace. Init waits up to about 15 seconds for `.jscad-studio` to appear with a live pid. If the server process exits before that, init fails at once with the log's last lines, instead of waiting out the full 15 seconds; on a real timeout it kills the server process and prints the log's last lines, then exits 1.
...
4. Runs the agent in the workspace with a prompt to read `AGENTS.md` and start on the model: `claude "<prompt>"`, or `opencode --prompt "<prompt>"` with `--opencode`. If that command is not on `PATH`, init prints the viewer URL and a note that the server keeps running, naming the command it could not find, then exits 0. While the agent runs, a Ctrl+C in the terminal goes to the agent, which uses it to interrupt what it is doing; init ignores it and keeps waiting for the agent to exit, so it does not die and orphan the server. A closed terminal (SIGHUP) ends both the same way.
5. When the agent exits, init stops the work server, but only if it started that server itself (a reused server is left running) and `.jscad-studio` still names the pid init started; if some other run of `jscad-work` has since replaced it, init leaves it alone. Init then exits with the agent's exit status, or 1 if the agent was killed by a signal.
...
`docs/reference/jscad-fluent-llm.txt` is a copy of `../jscad-fluent/llm.txt`, which `JSCAD.md` falls back to when the agent cannot fetch the upstream file. `npm run sync-llm` copies the upstream file over it and exits 1 when `../jscad-fluent` is not beside this repo. `test/llm-sync.test.js` fails with `run npm run sync-llm` when the two differ.
