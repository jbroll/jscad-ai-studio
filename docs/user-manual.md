...
4. Runs `claude` in the workspace with a prompt to read `AGENTS.md` and start on the model. If `claude` is not on `PATH`, it prints the viewer URL and a note that the server keeps running, then exits 0. While claude runs, a Ctrl+C in the terminal goes to claude, which uses it to interrupt what it is doing; init ignores it and keeps waiting for claude to exit, so it does not die and orphan the server. A closed terminal (SIGHUP) ends both the same way.
5. When claude exits, init stops the work server, but only if it started that server itself (a reused server is left running) and `.jscad-studio` still names the pid init started; if some other run of `jscad-work` has since replaced it, init leaves it alone. Init then exits with claude's exit status, or 1 if claude was killed by a signal.
