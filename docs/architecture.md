...
`lib/init.js` gives `jscad-work init` its one-command behavior: `resolveWorkspace` turns the argument into a workspace directory and model name, `scaffoldWorkspace` writes the prompt files, and `runInit` then starts the work server in the background (reusing one already running, and racing the spawned server process's exit against a startup timeout), waits for `.jscad-studio`, opens the viewer URL in a browser, and runs `claude` in the workspace, ignoring the terminal signals that would otherwise kill `init` while `claude` handles them itself. When `claude` exits, `runInit` stops the server only if it started that server and `.jscad-studio` still names its pid. Each step is an injected function, so `bin/jscad-work.js` wires the real server spawn, browser open, and `claude` invocation, and tests pass fakes to check the order without starting a real browser or agent.
